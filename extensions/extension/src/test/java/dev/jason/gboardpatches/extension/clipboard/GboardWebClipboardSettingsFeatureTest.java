...
public final class GboardWebClipboardSettingsFeatureTest {
    ...
    public void webServerPortRemainsEditableWhenWebClipboardIsDisabled() {
        ...
        GboardPatchesSettingsContract.ActionRow portRow =
                findActionRow(screen, "Web server port:");
    ...
    public void headerCarriesTileRecommendationAndEnableRowDoesNotRepeatIt() {
        ...
        Assert.assertEquals("",
                ((GboardPatchesSettingsContract.SwitchRow) screen.getRows().get(0)).getSummary());
    }

    private static GboardPatchesSettingsContract.ActionRow findActionRow(
            GboardPatchesSettingsContract.Screen screen,
            String titlePrefix) {
        for (GboardPatchesSettingsContract.Row row : screen.getRows()) {
            if (row instanceof GboardPatchesSettingsContract.ActionRow actionRow
                    && row.getTitle().startsWith(titlePrefix)) {
                return actionRow;
            }
        }
        throw new AssertionError("Missing action row with title prefix: " + titlePrefix);
    }
